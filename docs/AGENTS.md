# Multi-Agent Architecture — Microsoft Fabric to Gateway Migration
...
| **@extractor** | Parsing Microsoft Fabric source artifacts | `src/` extraction modules |
...
| **@generator** | Cross-cutting generation coordination | (output) generators |
...
```
        ...
        │(Microsoft Fabric parse)│  │ (coord.)  │   │ (coord.)   │
        └──────┬─────┘   └─────┬─────┘   └─────┬──────┘
               │          ┌────┴────┐     ┌─────┴──────┐
               │          │         │     │            │
               │     ┌────▼───┐ ┌───▼───┐ ┌▼────────┐ ┌▼──────┐
...
| **Formula Correctness** | Valid syntax, correct conversion from Microsoft Fabric formulas |
